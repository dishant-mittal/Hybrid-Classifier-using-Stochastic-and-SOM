...
xtest = data[0:560,0:9].astype(np.float)
ytest = data[0:560,9].astype(np.int)
...
flag=0
flag1=0
scatter(data[0:560,0],ytest, s=220, facecolors='none', edgecolors='r',label="actual value")
for i in range(0,560):
	if int(ytest[i]) == int(p[i]):
		if flag ==0:
			scatter(data[i,0],p[i],s=140,color='lightskyblue',alpha=0.4,label=" correct prediction")
			flag=1
		else:
			scatter(data[i,0],p[i],s=140,color='lightskyblue',alpha=0.4)
for i in range(0,560):
	if int(ytest[i]) != int(p[i]):
		if flag1== 0:
			scatter(data[i,0],p[i],s=180,marker='x',color='black',alpha=0.9,label="incorrect prediction")
			flag1=1
		else:
			scatter(data[i,0],p[i],s=180,marker='x',color='black',alpha=0.9)
