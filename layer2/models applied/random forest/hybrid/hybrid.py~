...
csv_file = csv.reader(open('intermediate_layer.csv'))
...
for i in range(0,698):
	data[i,4] = data[i,4].replace('2',"0")
	data[i,4] = data[i,4].replace('4',"1")

xtrain = data[0:560,0:4].astype(np.float)
ytrain = data[0:560,4].astype(np.int)
xtest = data[560:699,0:4].astype(np.float)
ytest = data[560:699,4].astype(np.int)
...
flag=0
flag1=0
scatter(data[560:699,5],ytest, s=220, facecolors='none', edgecolors='r',label="actual value")
for i in range(0,138):
	if int(ytest[i]) == int(p[i]):
		if flag ==0:
			scatter(data[i+560,5],p[i],s=140,color='lightskyblue',alpha=0.4,label=" correct prediction")
			flag=1
		else:
			scatter(data[i+560,5],p[i],s=140,color='lightskyblue',alpha=0.4)
for i in range(0,138):
	if int(ytest[i]) != int(p[i]):
		if flag1== 0:
			scatter(data[i+560,5],p[i],s=180,marker='x',color='black',alpha=0.9,label="incorrect prediction")
			flag1=1
		else:
			scatter(data[i+560,5],p[i],s=180,marker='x',color='black',alpha=0.9)
...
title('Nearest Centroid Classifier',size=18)
